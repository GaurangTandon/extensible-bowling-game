...
class AddPartyView extends WindowView implements ListSelectionListener {
    private static final String ERR_MEMBER_EXISTS = "Member already in Bowlers.Party";
    ...
    AddPartyView(final ControlDeskView controlDeskView) {
        super("Add Bowlers.Party");
        party = new ArrayList<>(0);
        controlDesk = controlDeskView;

        partyPanel = drawScrollable("(Empty)", "Your Bowlers.Party", 5)
                .attachListener(this);
        bowlerPanel = drawScrollable(BowlerFile.getBowlers(), "Bowlers.Bowler Database", 8)
                .attachListener(this);
    ...
    void updateNewPatron(final NewPatronView newPatron) {
        ...
        if (res != null) {
            ...
            System.err.println("A Bowlers.Bowler with that name already exists.");
        }
    }
